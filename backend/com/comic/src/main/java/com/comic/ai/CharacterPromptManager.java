...
public class CharacterPromptManager {
    ...
    public String buildThreeViewGridPrompt(Character character, VisualStyle style) {
        ...
        return stylePrefix +
                ...
                "属性特征：" + personality + "\n\n" +
                "构图要求：\n" +
                "- 横向布局，从左到右依次排列3个视图：最左侧为正面视图、中间为侧面视图（侧面轮廓）、最右侧为背面视图\n" +
                "- 严禁调换视图排列顺序，严禁自行决定位置，必须严格按照左=正面、中=侧面、右=背面排列\n" +
                postureReq +
                "- 纯色平铺背景（白色、浅灰色或黑色），不要图案、渐变、环境元素\n" +
                ...
                "4. 平静表情——所有视图使用中性面部表情\n" +
                "5. 清晰对齐——正面、侧面和背面视图水平对齐，比例一致\n" +
                "6. **必须从左到右排列**——正面在左、侧面居中、背面在右，这是最高优先级要求\n\n" +
                "负面提示词：" + negativePrompt;
    }
        ...
        switch (style) {
            case REAL:
                return "写实风格，照片级真人肖像，电影级摄影质感，" +
                       ...
                       "角色默认为东亚面孔特征（除非角色设定中明确描述了其他种族外貌）。\n\n";
            case D_3D:

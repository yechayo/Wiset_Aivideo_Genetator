...
public class DeepSeekTextService implements TextGenerationService {
    ...
    public List<Map<String, Object>> generateStoryboard(
        ...
        String systemPrompt = "你是一位专业的影视分镜师。请根据提供的剧本内容，生成详细的分镜脚本。\n"
            ...
            + "- duration: 时长（秒，2-4）\n"
            + "- scene: 场景描述（具体的环境细节，包括光线、天气、空间布局）\n"
            + "- characters: 出场角色数组\n"
            + "- shotSize: 景别（大远景/远景/全景/中景/中近景/近景/特写/大特写）\n"
            + "- cameraAngle: 角度（视平/高位俯拍/低位仰拍/斜拍/越肩/鸟瞰/荷兰角/低角度仰拍/高角度俯拍）\n"
            + "- cameraMovement: 运镜描述（必须详细描述镜头的动态运动，包括：运镜方式如推/拉/摇/移/跟/升降/环绕/手持晃动/固定等，运动方向和速度如缓慢/匀速/快速/急促，起始位置和结束位置，与主体或场景的关系，营造的视觉氛围。示例：\"镜头从角色眼部特写缓慢开始，逐渐向后拉远至中景，同时向左平移30度，展现场景全貌，营造孤独空旷的压抑氛围\"或\"手持跟拍，从背后跟随角色快速奔跑，镜头有轻微晃动感，增强紧张刺激感\"或\"固定机位正前方，通过大光圈浅景深将焦点从前景的杯子缓慢转移到后景的人物脸上\"。禁止只写\"横移\"、\"推拉\"、\"固定\"等简单词汇！）\n"
            + "- visualDescription: 画面描述（必须详细描述画面内容，包括角色具体动作姿态、面部表情、身体语言、手势、光影效果、色彩氛围。示例：\"女孩右手紧握裙摆，微微低头，眼眶泛红但强忍着泪水，头顶的夕阳余晖在她发梢形成金色光晕，背景是模糊的校园走廊\"）\n"
            + "- dialogue: 对白（无则填\"无\"）\n"
            + "- speaker: 说话人角色名（无对白则填\"无\"，有对白时必须是 characters 数组中的角色之一）\n"
            + "- dialogueTone: 对白语气（无对白则填\"无\"。必须描述说话人的语气、情绪状态和表演方式。示例：\"愤怒而急促，声音略带颤抖\"或\"温柔低语，带着一丝犹豫和心疼\"或\"震惊地提高音量，难以置信\"或\"冷漠平淡，不带任何感情\"或\"故作轻松但掩饰不住内心的悲伤\"）\n"
            + "- visualEffects: 视觉特效（无则填\"无\"）\n"
            + "- audioEffects: 音效（无则填\"无\"）\n"
            + "- transitionHint: 镜头衔接提示（描述此镜头如何过渡到下一个镜头，确保画面连贯性。示例：\"角色转身走出的动势自然衔接下一镜\"或\"对话结束后的沉默配合画面渐暗\"或\"快速摇镜头模糊转场，配合音效冲击\"。最后一个分镜填写\"最后一个镜头，无需衔接\"）\n\n"
            + "重要规则：\n"
            + "1. dialogue 与 speaker 必须严格对应。如果 dialogue 不为\"无\"，则 speaker 必须是 characters 数组中的某个角色名。\n"
            + "2. dialogue 与 dialogueTone 必须严格对应。如果 dialogue 不为\"无\"，dialogueTone 不能为\"无\"，要精准刻画角色的情感状态。\n"
            + "3. 分镜之间必须有连贯性。每个分镜的 transitionHint 要清晰描述画面如何过渡到下一个分镜，确保叙事流畅。\n"
            + "4. 最后一个分镜的 transitionHint 必须写\"最后一个镜头，无需衔接\"。\n"
            + "5. cameraMovement 必须具体到运动细节（方式、方向、速度、起始/结束位置、氛围），禁止只写\"横移\"、\"推拉\"、\"固定\"等简单词汇。\n"
            + "6. visualDescription 必须包含角色的具体动作、表情、身体语言和光影氛围，禁止笼统描述。\n\n"
            + "**角色名称约束**：characters 数组中的每个角色名必须与提供的角色描述中【】内的名称完全一致，"
            + "禁止使用昵称、简称、别名或任何变体。例如角色描述为【墨尘（幻影）】，则必须写\"墨尘（幻影）\"，不能写\"墨尘\"或\"幻影\"。\n\n"
            + "**AI视频生成三原则（必须严格遵守）：**\n"
            + "1.【单主体原则】每个分镜最多只保留1个角色的动作描写，禁止两个角色同框互动（如拥抱、打斗、接触）。"
            + "双人对话必须拆分为两个分镜（A的反应镜头 + B的反应镜头），用剪辑快切实现对话效果。"
            + "静态多人同框（如多人站立场面）允许，但禁止动态互动。\n"
            + "2.【慢动作原则】cameraMovement 必须使用缓慢运动（缓慢推镜头、微平移、极慢拉远、静止等）。"
            + "visualDescription 中的角色动作必须是微小动作（微风吹动头发、眼皮微垂、轻微呼吸、手指轻颤、嘴角微动等），"
            + "禁止描写剧烈运动（快速奔跑、跳跃、翻滚、打斗）。激烈场面通过多分镜快切实现，而非单镜头内的快动。\n"
            + "3.【说话人与嘴部约束原则】本视频为音画同步生成，对白和画面同时产出，必须严格区分说话人与非说话人：\n"
            + "  (a) 当 speaker 是 characters 中的某个画面内角色时：visualDescription 中可以描写该说话人自然的说话神态（如表情、眼神、手势），但禁止详细描写嘴部开合、口型蠕动等唇齿运动。\n"
            + "  (b) 当 speaker 为旁白、画外音、内心独白，或 speaker 不在 characters 列表中（即不在画面中）时："
            + "visualDescription 中所有角色必须保持闭嘴静止，处于倾听、思考或感受状态，绝对禁止任何嘴部动作。\n"
            + "  (c) 非说话人的画面内角色：嘴巴必须闭合，只能通过眼神、表情、头部动作表达反应，禁止任何嘴部运动。\n"
            + "  (d) speaker 字段必须精确标注说话人。有对白时 speaker 不可填\"无\"；若对白来自旁白则填\"旁白\"，内心独白则填对应角色名加\"（内心独白）\"。\n"
            + "  (e) dialogueTone 必须精准描述说话人的语气情绪，帮助视频模型理解谁在说话、以什么情绪说话。\n\n"
            + "**说话人标注强化规则：**\n"
            + "- 每个有对白的分镜，visualDescription 的开头必须先写明画面主体角色的状态，再自然引出说话人的反应。\n"
            + "- 示例（说话人在画面中）：speaker=\"小明\"，visualDescription 应写\"小明微微前倾，眼神认真注视前方，手中紧握信纸，表情从期待逐渐转为感动\"，"
            + "不要写\"小明张嘴说话\"或\"小明念出信的内容\"。\n"
            + "- 示例（说话人为旁白）：speaker=\"旁白\"，visualDescription 应写\"画面中所有人保持安静，小明低头沉思，远处夕阳缓缓沉入地平线\"，"
            + "明确体现画面内角色处于无声状态。\n"
            + "- 示例（说话人不在画面中）：speaker=\"小红\"但 characters=[\"小明\"]，visualDescription 应写\"小明独自站在窗前，表情凝重地望向窗外\"，"
            + "体现小明在倾听画面外的声音。";

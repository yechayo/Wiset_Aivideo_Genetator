...
/**
 * Grok 视频生成服务
 * 使用无因科技 Grok API 实现文生视频
 *
 * API 文档：
 * - 提交任务：POST https://api.wuyinkeji.com/api/async/video_grok_imagine
 * - 查询任务：GET  https://api.wuyinkeji.com/api/async/detail
 */
@Service
@Slf4j
@RequiredArgsConstructor
public class GrokVideoService implements VideoGenerationService {

    private static final String SUBMIT_ENDPOINT = "/api/async/video_grok_imagine";
    private static final String QUERY_ENDPOINT = "/api/async/detail";
    ...
    public String generateAsync(String prompt, int duration, String aspectRatio, String referenceImage, boolean offPeak) {
        return generateAsync(prompt, duration, aspectRatio, referenceImage, offPeak, null);
    }
    ...
    public String generateAsync(String prompt, int duration, String aspectRatio, String referenceImage, boolean offPeak, String model) {
        ...
        try {
            semaphore.acquire();
            acquired = true;
            log.info("Grok 视频生成: 并发槽位 {}/{}", semaphore.availablePermits(), semaphore.getQueueLength());

            // 默认值处理
            String safeAspectRatio = (aspectRatio != null && !aspectRatio.trim().isEmpty()) ? aspectRatio : "2:3";
            ...
            requestBody.put("aspect_ratio", safeAspectRatio);

            // 参考图（可选）- Grok 使用 image_urls 数组格式
            if (referenceImage != null && !referenceImage.trim().isEmpty()) {
                requestBody.put("image_urls", new String[]{referenceImage});
            }

            String jsonBody = objectMapper.writeValueAsString(requestBody);
            log.debug("Grok 请求体: {}", jsonBody);
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                if (!response.isSuccessful()) {
                    ...
                    log.error("Grok API 调用失败: {} - {}", response.code(), errorBody);
                    throw new RuntimeException("Grok 视频生成失败: " + response.code() + " - " + errorBody);
                }

                String responseBody = response.body().string();
                log.debug("Grok 响应: {}", responseBody);

                JsonNode root = objectMapper.readTree(responseBody);
                int code = root.has("code") ? root.get("code").asInt() : -1;
                if (code != 200) {
                    String msg = root.has("msg") ? root.get("msg").asText() : responseBody;
                    log.error("Grok API 返回错误: code={}, msg={}", code, msg);
                    throw new RuntimeException("Grok 视频生成失败: " + msg);
                }
                ...
                log.info("Grok 视频生成任务已提交: taskId={}", taskId);
                return taskId;
            }

        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("Grok 视频生成被中断", e);
        } catch (IOException e) {
            log.error("Grok 视频生成 IO 异常", e);
            throw new RuntimeException("Grok 视频生成失败: " + e.getMessage(), e);
        } catch (Exception e) {
            log.error("Grok 视频生成异常", e);
            throw new RuntimeException("Grok 视频生成失败: " + e.getMessage(), e);
        } finally {
            if (acquired) {
                semaphore.release();
    ...
    public TaskStatus getTaskStatus(String taskId) {
        try {
            String url = wuyinkejiProperties.getBaseUrl() + QUERY_ENDPOINT + "?key=" + wuyinkejiProperties.getApiKey() + "&id=" + taskId;
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                if (!response.isSuccessful()) {
                    String errorBody = response.body() != null ? response.body().string() : "无响应体";
                    log.warn("查询 Grok 任务状态失败: code={}, taskId={}, error={}", response.code(), taskId, errorBody);
                    return new TaskStatus(taskId, "unknown", 0, null, "查询失败: " + response.code());
                }

                String responseBody = response.body().string();
                log.debug("Grok 任务状态响应: {}", responseBody);
                return parseTaskStatus(responseBody, taskId);
            }
        } catch (IOException e) {
            log.error("查询 Grok 任务状态 IO 异常: taskId={}", taskId, e);
    ...
    private TaskStatus parseTaskStatus(String responseBody, String taskId) {
        try {
            ...
            if (code != 200) {
                ...
                log.warn("Grok 查询返回非 200: code={}, msg={}, taskId={}", code, msg, taskId);
            ...
            log.error("解析 Grok 任务状态失败: {}", responseBody, e);

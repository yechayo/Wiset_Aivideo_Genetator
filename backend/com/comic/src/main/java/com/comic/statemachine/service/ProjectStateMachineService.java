...
public class ProjectStateMachineService {
    ...
    private StateMachine<ProjectState, ProjectEventType> createStateMachine(String projectId) {
        ...
        if (project != null) {
            ...
            if (currentStatus != null) {
                try {
                    ProjectState currentState = ProjectState.valueOf(currentStatus);
                    ...
                    log.warn("Unknown project status in DB: {}", currentStatus);
                }
            }
        }
    ...
    public boolean sendEvent(String projectId, ProjectEventType event, Map<String, Object> headers) {
        try {
            ...
            log.info("Sending event: projectId={}, event={}", projectId, event);
            ...
            if (!accepted) {
                log.error("Event NOT ACCEPTED: projectId={}, event={}, currentState={}",
    ...
    public void persistState(String projectId, ProjectState newState) {
        ...
        if (!newState.name().equals(oldStatus)) {
            ...
            // 发布状态变更事件
            eventPublisher.publishStateChange(projectId, newState);
        } else {

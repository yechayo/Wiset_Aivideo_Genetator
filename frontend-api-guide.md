...
| page | number | 1 | 页码 |
| size | number | 10 | 每页数量 |
| role | string | - | 角色类型筛选（主角 / 反派 / 配角） |
| name | string | - | 角色名称模糊搜索 |

**响应 `data`（CharacterListItemResponse）：**
...
| visualStyle | string | 视觉风格（3D / ANIME / COMIC） |
| expressionStatus | string | 表情生成状态（pending / generating / completed / failed） |
...
| expressionStatus | string | 表情生成状态（pending / generating / completed / failed） |
| threeViewStatus | string | 三视图生成状态（pending / generating / completed / failed） |
| expressionError | string | 表情生成失败时的错误信息 |
| threeViewError | string | 三视图生成失败时的错误信息 |
| isGeneratingExpression | boolean | 是否正在生成表情 |
| isGeneratingThreeView | boolean | 是否正在生成三视图 |
| expressionGridUrl | string | 九宫格大全图 URL |
| threeViewGridUrl | string | 三视图大全图 URL |
...
SCRIPT_CONFIRMED → CHARACTER_EXTRACTING    ← 原子自动推进（同一事务）
  ↓ （异步触发角色提取）
CHARACTER_REVIEW
  ↓ 用户调用 POST /characters/confirm
CHARACTER_CONFIRMED → IMAGE_GENERATING     ← 原子自动推进（同一事务）
  ↓ （异步触发图片生成）
IMAGE_REVIEW
  ↓ 用户调用 POST /characters/images/confirm
ASSET_LOCKED → PANEL_GENERATING            ← 原子自动推进（同一事务）
  ↓ （异步触发分镜生成）
PANEL_REVIEW
...
> **原子自动推进说明：** `SCRIPT_CONFIRMED → CHARACTER_EXTRACTING`、`CHARACTER_CONFIRMED → IMAGE_GENERATING`、`ASSET_LOCKED → PANEL_GENERATING` 这三组转换在同一个事务中完成，前端 SSE 或轮询时只会看到最终的 `*_GENERATING` 状态，不会看到中间的 `CONFIRMED` / `LOCKED` 过渡态。

---

### 失败状态与重试

| 失败/卡死状态 | 所在步骤 | 重试方式 | 重试后回到 |
|---|---|---|---|
| OUTLINE_GENERATING_FAILED | 2 | `{direction:"forward",event:"retry"}` | DRAFT |
| OUTLINE_GENERATING（卡死） | 2 | 同上 | DRAFT |
| EPISODE_GENERATING_FAILED | 2 | 同上 | OUTLINE_REVIEW |
| EPISODE_GENERATING（卡死） | 2 | 同上 | OUTLINE_REVIEW |
| CHARACTER_EXTRACTING_FAILED | 3 | 同上 | SCRIPT_CONFIRMED |
| CHARACTER_EXTRACTING（卡死） | 3 | 同上 | SCRIPT_CONFIRMED |
| IMAGE_GENERATING_FAILED | 4 | 同上 | CHARACTER_CONFIRMED |
| IMAGE_GENERATING（卡死） | 4 | 同上 | CHARACTER_CONFIRMED |
| PANEL_GENERATING_FAILED | 5 | 同上 | ASSET_LOCKED |
| PANEL_GENERATING（卡死） | 5 | 同上 | ASSET_LOCKED |

> **卡死恢复：** 如果项目停留在 `*_GENERATING` 中间态超过合理时间（例如服务器重启导致回调丢失），也可以通过 `retry` 事件恢复到前一个审核态。
...
| `revise_episodes` | 重新生成剧集 | `POST /script/episodes/revise` |
| `confirm_script` | 确认剧本 | `POST /script/confirm` |
| `update_character` | 编辑角色 | `PUT /characters/{charId}` |

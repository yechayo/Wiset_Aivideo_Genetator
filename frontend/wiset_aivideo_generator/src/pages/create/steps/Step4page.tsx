 ...
 * Step4page: 分镜生产工作台
 *
 * 流水线：分镜文本审核 → 九宫格生成 → 九宫格审核 → 视频生成
 * 展示三级可展开列表（章节 → 集数 → 片段），
 * 顶部显示完成进度统计栏。
 */
const Step4page = ({ project, onNextStep }: Step4pageProps) => {
  ...
  // 文本自动生成：已触发标记（防止重复触发）
  const textAutoTriggeredRef = useRef(false);
  // 九宫格生成中标记
    ...
    try {
      ...
      for (const [chapterTitle, chapterEpisodes] of chapterMap.entries()) {
        ...
        const episodeStates: EpisodeState[] = chapterEpisodes.map((ep, idx) => {
          ...
          // text_ready 阶段：从 shots 构建审核用的 segments（无 panels）
          const textSegments: SegmentState[] = [];
          if (ep.episodeInfo?.gridStatus === 'text_ready' && Array.isArray(ep.episodeInfo?.shots)) {
            ep.episodeInfo.shots.forEach((shot: any, sIdx: number) => {
              textSegments.push({
                ...
                sceneThumbnail: null,
                characterAvatars: (shot.characters || []).map((name: string) => ({
                  charId: charNameToIdMap[name] || '',
                  name,
                  avatarUrl: '',
                })),
                pipelineStep: 'pending' as SegmentPipelineStep,
                gridImages: [],
                gridStatus: 'pending',
                fusionImageUrl: null,
                shots: [shot],
  ...
  /**
   * 自动触发文本生成：当 availableActions 包含 generate_panels 且尚未触发时自动调用
   */
  useEffect(() => {
    if (!projectId || textAutoTriggeredRef.current) return;
    const hasGeneratePanels = statusInfo?.availableActions?.includes('generate_panels');
    if (hasGeneratePanels && !statusInfo?.isGenerating) {
      textAutoTriggeredRef.current = true;
      (async () => {
        try {
          await advanceStatus(projectId, 'forward', 'generate_panels');
          await syncStatus(projectId);
          loadEpisodes();
        } catch (e: any) {
          setError(e.message || '生成分镜文本失败');
          textAutoTriggeredRef.current = false;
        }
      })();
    }
  }, [projectId, statusInfo?.availableActions, statusInfo?.isGenerating, advanceStatus, syncStatus, loadEpisodes]);

  /**
   * 触发九宫格图片生成（人工审核分镜文本后调用）
   */
  const handleGenerateGrids = useCallback(async () => {
    if (!projectId || generatingGrids) return;
    setGeneratingGrids(true);
    try {
      await advanceStatus(projectId, 'forward', 'generate_grids');
      await syncStatus(projectId);
    } catch (e: any) {
      setError(e.message || '生成九宫格失败');
    } finally {
      setGeneratingGrids(false);
    }
  }, [projectId, generatingGrids, advanceStatus, syncStatus]);
    ...
    if (hasGeneratePanels && !statusInfo?.isGenerating) {
      ...
      (async () => {
        try {
          ...
          setError(e.message || '生成分镜文本失败');
          textAutoTriggeredRef.current = false; // 失败后允许重试
        }
      })();
    }
  }, [projectId, statusInfo?.availableActions, statusInfo?.isGenerating, advanceStatus, syncStatus, loadEpisodes]);
  ...
  // ========== 分镜文本审核阶段 ==========
  // 当集数存在且 gridStatus === 'text_ready' 时，展示分镜文本供审核
  const hasTextReady = chapters.some(ch => ch.episodes.some(ep => ep.gridStatus === 'text_ready'));
  const isStoryboardReviewPhase = hasTextReady;

  if (isStoryboardReviewPhase && chapters.length > 0) {
      ...
      <div className={styles.pageContainer}>
        <div className={styles.pageHeader}>
          <div className={styles.titleSection}>
            ...
            <p className={styles.pageSubtitle}>审核分镜脚本，确认后生成九宫格分镜图</p>
          </div>
        </div>
        ...
        <div style={{ padding: '12px 0', display: 'flex', gap: '12px' }}>
          <button
            ...
            disabled={generatingGrids}
            style={{
              padding: '8px 20px', borderRadius: 8,
              background: generatingGrids ? '#999' : '#1677ff',
              color: '#fff', border: 'none', cursor: generatingGrids ? 'not-allowed' : 'pointer',
            }}
          >
            {generatingGrids ? '九宫格生成中...' : '全部通过，开始生成九宫格'}
          </button>
        </div>
        ...
        <div className={styles.chapterList}>
          {chapters.map(chapter => {
            const chapterEps = chapter.episodes.filter(ep => ep.gridStatus === 'text_ready');
            if (chapterEps.length === 0) return null;
            return (
              <div key={chapter.chapterIndex} className={styles.chapterGroup}>
                ...
                {chapterEps.map(ep => {
                  return (
                    <div key={ep.episodeId} style={{ marginBottom: 24, padding: '16px', background: '#fff', borderRadius: 8, border: '1px solid #f0f0f0' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <h3 style={{ margin: 0 }}>第{ep.episodeIndex}集 {ep.title}</h3>
                        <span style={{ fontSize: 13, color: '#999' }}>
                          {ep.segments.length > 0 ? `${ep.segments.length} 个分镜` : '分镜数据加载中...'}
                        </span>
                      </div>
                      {/* 分镜文本来自 segments（已通过 loadPanelsForEpisode 加载） */}
                      {ep.segments.length > 0 ? ep.segments.map((seg, idx) => (
                        <div key={idx} style={{
                          padding: '12px 16px', marginBottom: 8, background: '#fafafa',
                          ...
                          {seg.synopsis && (
                            <div style={{ marginBottom: 4 }}>
                              <span style={{ color: '#666', fontSize: 13 }}>概要：</span>
                              <span style={{ fontSize: 13 }}>{seg.synopsis}</span>
                            </div>
                          )}
                          {seg.panelData?.composition && (
                            <div style={{ marginBottom: 4 }}>
                              <span style={{ color: '#666', fontSize: 13 }}>构图：</span>
                              <span style={{ fontSize: 13 }}>{seg.panelData.composition}</span>
                            </div>
                          )}
                          {seg.panelData?.dialogue && (
                            <div style={{ marginBottom: 4 }}>
                              <span style={{ color: '#666', fontSize: 13 }}>对话：</span>
                              <span style={{ fontSize: 13, whiteSpace: 'pre-wrap' }}>{seg.panelData.dialogue}</span>
                            </div>
                          )}
                          {seg.characterAvatars.length > 0 && (
                            <div>
                              <span style={{ color: '#666', fontSize: 13 }}>角色：</span>
                              <span style={{ fontSize: 13 }}>{seg.characterAvatars.map(a => a.name).join('、')}</span>
                            </div>
                          )}
                        </div>
                      )) : (
  ...
  return (
    <div className={styles.pageContainer}>
      ...
      <div className={styles.pageHeader}>
        <div className={styles.titleSection}>
          <h1 className={styles.pageTitle}>分镜生产</h1>
          <p className={styles.pageSubtitle}>分镜文本审核、九宫格生成与视频生产</p>
        </div>
